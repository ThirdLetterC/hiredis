...
enum connection_type { CONN_TCP, CONN_UNIX, CONN_FD, CONN_SSL };
...
static void millisleep(int ms) { usleep(ms * 1000); }
...
static void test_format_commands(void) {
  ...
  len = redisFormatCommand(&cmd, "SET %b %b", "foo", (size_t)3, "b\0r",
                           (size_t)3);
  test_cond(strncmp(cmd, "*3\r\n$3\r\nSET\r\n$3\r\nfoo\r\n$3\r\nb\0r\r\n",
                    len) == 0 &&
            len == 4 + 4 + (3 + 2) + 4 + (3 + 2) + 4 + (3 + 2));
...
#define FLOAT_WIDTH_TEST(type)                                                 \
  ...
  test("Format command with unhandled printf format (specifier 'p' not "
       "supported): ");
...
static void test_tcp_options(struct config cfg) {
  ...
  test_cond(redisSetTcpUserTimeout(c, 100) == REDIS_ERR &&
            c->err == REDIS_ERR_IO);
#endif
...
static void test_reply_reader(void) {
  ...
  ret = redisReaderGetReply(reader, &reply);
  test_cond(ret == REDIS_ERR &&
            strcasecmp(reader->errstr, "Out of memory") == 0);
  freeReplyObject(reply);
  redisReaderFree(reader);
  ...
  ret = redisReaderGetReply(reader, &reply);
  test_cond(
      ret == REDIS_OK && ((redisReply *)reply)->type == REDIS_REPLY_DOUBLE &&
  ...
  test_cond(
      ret == REDIS_OK && ((redisReply *)reply)->type == REDIS_REPLY_DOUBLE &&
      isinf(((redisReply *)reply)->dval) && ((redisReply *)reply)->dval > 0);
  freeReplyObject(reply);
  redisReaderFree(reader);
  ...
  ret = redisReaderGetReply(reader, &reply);
  test_cond(ret == REDIS_OK && ((redisReply *)reply)->type == REDIS_REPLY_NIL);
  freeReplyObject(reply);
  redisReaderFree(reader);
  ...
  redisReaderFeed(reader, "%2\r\n+first\r\n:123\r\n$6\r\nsecond\r\n#t\r\n", 34);
  ret = redisReaderGetReply(reader, &reply);
  test_cond(ret == REDIS_OK && ((redisReply *)reply)->type == REDIS_REPLY_MAP &&
  ...
  redisReaderFeed(reader,
                  "~5\r\n+orange\r\n$5\r\napple\r\n#f\r\n:100\r\n:999\r\n", 40);
  ret = redisReaderGetReply(reader, &reply);
  test_cond(ret == REDIS_OK && ((redisReply *)reply)->type == REDIS_REPLY_SET &&
  ...
  test_cond(ret == REDIS_OK &&
            ...
            fabs(((redisReply *)reply)->element[0]->dval -
                 3.14159265358979323846) < 0.00000001 &&
            ((redisReply *)reply)->element[0]->len == 22 &&
            strcmp(((redisReply *)reply)->element[0]->str,
...
static void test_resp3_push_options(struct config config) {
  ...
  test("We can use redisOptions to set a custom PUSH handler for "
       "redisContext: ");
  options = get_redis_tcp_options(config);
...
static void test_blocking_connection(struct config config) {
  ...
  test_cond(reply->type == REDIS_REPLY_STATUS &&
            strcasecmp(reply->str, "pong") == 0) freeReplyObject(reply);
  ...
  reply = redisCommand(c, "SET %b %b", "foo", (size_t)3, "hello\x00world",
                       (size_t)11);
  freeReplyObject(reply);
  reply = redisCommand(c, "GET foo");
  test_cond(reply->type == REDIS_REPLY_STRING &&
...
static void test_blocking_connection_timeouts(struct config config) {
  ...
  if (detect_debug_sleep(c)) {
    ...
    // wait for the DEBUG SLEEP to complete so that Redis server is unblocked
...
static void test_invalid_timeout_errors(struct config config) {
  ...
  test_cond(c != NULL && c->err == REDIS_ERR_IO &&
            strcmp(c->errstr, "Invalid timeout specified") == 0);
  redisFree(c);
...
// }
//
// static void __test_reply_callback(redisContext *c, redisReply *reply, void
// *privdata) {
//     ((void)c);
//     /* Shift to detect execution order */
//     __test_callback_flags <<= 8;
...
#pragma GCC diagnostic ignored                                                 \
    "-Woverlength-strings" /* required on gcc 4.8.x due to assert statements   \
                            */
...
void subscribe_channel_a_cb(redisAsyncContext *ac, void *r, void *privdata) {
  ...
  if (strcmp(reply->element[0]->str, "subscribe") == 0) {
    ...
    /* Unsubscribe to channels, including channel X & Z which we don't subscribe
     * to */
    redisAsyncCommand(ac, unexpected_cb,
...
static void commandCallback(struct redisAsyncContext *ac, void *_reply,
                            void *_privdata);
...
int main(int argc, char **argv) {
  struct config cfg = {.tcp = {.host = "127.0.0.1", .port = 6379},
  ...
  if (cfg.ssl.port && cfg.ssl.host) {
    ...
    redisInitOpenSSL();
    _ssl_ctx = redisCreateSSLContext(cfg.ssl.ca_cert, NULL, cfg.ssl.cert,
                                     cfg.ssl.key, NULL, NULL);
