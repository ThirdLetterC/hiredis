 ...
 *       gcc -fPIC -shared -I../../redis/src/ -I.. example-redismoduleapi.c -o
 * example-redismoduleapi.so
 *
 * 2- Load module:
...
int RedisModule_OnLoad(RedisModuleCtx *ctx, RedisModuleString **argv,
  ...
  int ret =
      RedisModule_Init(ctx, "example-redismoduleapi", 1, REDISMODULE_APIVER_1);
  ...
  usage of the adapter. Then in `getCallback`, we start a `debug sleep` command
  to create 1.5 second long request. Because we have set a 1 second timeout to
  the connection, the command will always fail with a timeout error, which is
  shown in the `debugCallback`.
  */
