...
static int redisLibsdeventTimeoutHandler(sd_event_source *s, uint64_t usec,
                                         void *userdata) {
  ((void)s);
  ((void)usec);
...
static int redisLibsdeventHandler(sd_event_source *s, int fd, uint32_t event,
                                  void *userdata) {
  ((void)s);
  ((void)fd);
  redisLibsdeventEvents *e = (redisLibsdeventEvents *)userdata;
...
#define CHECK_DELETED()                                                        \
  if (e->state & REDIS_LIBSDEVENT_DELETED) {                                   \
    redisLibsdeventDestroy(e);                                                 \
    return 0;                                                                  \
  }

  if ((event & EPOLLIN) && e->context &&
      (e->state & REDIS_LIBSDEVENT_DELETED) == 0) {
...
static void redisLibsdeventSetTimeout(void *userdata, struct timeval tv) {
  ...
  if (!e->timerSource) {
    sd_event_add_time_relative(e->event, &e->timerSource, CLOCK_MONOTONIC, usec,
                               1, redisLibsdeventTimeoutHandler, e);
  } else {
    sd_event_source_set_time_relative(e->timerSource, usec);
  }
}

static int redisLibsdeventAttach(redisAsyncContext *ac,
                                 struct sd_event *event) {
  redisContext *c = &(ac->c);
